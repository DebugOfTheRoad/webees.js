var wb = wb || {
	...
	*/
	isJson:function (obj){	return typeof(obj) == "object" && Object.prototype.toString.call(obj).toLowerCase() == "[object object]" && !obj.length;    },
	/***** 判断是否为数组对象 *******
	* @param obj: 对象
	* @return  是否是数组对象 true/false
	*/
	isArr:function (obj){	return Object.prototype.toString.call(obj).toLowerCase() == "[object array]";    },
	/***** 得到一个 0~iMax 的随机数 *******
	* @param iMax: 生成的最大值
	* @param iMin: 生成的最小值
	* @return 生成的随机数
	*/
	getRandom:function (iMin,iMax){  return Math.round(Math.random()*(iMax-iMin))+iMin;	  },
	/***** 得到一组不重复的随机数 *******
	...
	*/
	addScroll:function (obj,fnBack){
		...
		function fnScroll(ev){
			ev= ev||window.event;
			var dir=true;
			if(ev.wheelDelta){	dir = ev.wheelDelta > 0? true : false;	}else {	dir = ev.detail < 0 ? true : false;		}
			fnBack(ev,dir);
	...
	* @return arr: 随机数的数组
	*/
	addZero:function(str,num){	for(var i=0,len=num-str.length;i<len;i++){	str='0'+str;}	return str;	},
	/****数组相减****
	* @param arr1: 被减数数组
	* @param arr2: 减数数组
	...
	cutTowArr:function(arr1,arr2){
		for(var i=0, l=arr1.length; i<l; i++){
			for(var j=0, len=arr2.length; j<len; j++){
				if(arr1[i] == arr2[j]){	arr1.splice(i,1);	i--;	break;	}
			}
		}
		return arr1;
	},
	...
	*/
	getArrShare:function(arr1,arr2){
		var arr=[];
		for(var i=0, l=arr1.length; i<l; i++){
			for(var j=0, len=arr2.length; j<len; j++){	if(arr1[i] == arr2[j]){	arr.push(arr1[i]);	}	}
		}
		return arr;
	},
	...
	*/
	mixArr:function(arr1,arr2){
		var arr=[], l= arr1.length > arr2.length ? arr1.length : arr2.length;
		for(var i=0; i<l; i++){
			if(arr1[i]){	arr.push(arr1[i]);	}
			if(arr2[i]){	arr.push(arr2[i]);	}
		}
		return arr;
	},
	/**
	...
	*/
	...
	setOpos:function(obj){
		var rx=obj.offsetWidth/2,	ry=obj.offsetHeight/2;
		obj.o={	x:obj.offsetLeft+rx,	y:obj.offsetTop+ry	};
	},
	/**
	...
	* @return 无
	*/
	delCookie:function(key) {	setCookie(key, '', -1);	},
	//删除指定class函数
	...
	addClass:function(obj,sClass){
		var aClass=obj.className.split(' ');
		if(!aClass[0]){ obj.className+=' '+sClass;	return false;	}
		for(var i=0;i<aClass.length;i++){	if(aClass[i]==sClass) return false;	}
	...
	*/
	getByClass:function(sClass,parent){
		var aEles = (parent||document).getElementsByTagName('*'),	arr = [];
		for(var i=0, l=aEles.length; i<l; i++){
			var aClass = aEles[i].className.split(' ');
			for(var j=0; j<aClass.length; j++){
				if(aClass[j] == sClass){	arr.push(aEles[i]);	break;	}
			}
		}
		return arr;
	}
...
Array.prototype.delReElement = Array.prototype.delReElement || function(){
	...
	for(var i=0, l=this.length; i<l;i++){
		if(!json[this[i]]){	arr.push(this[i]);	json[this[i]] = [this[i]];	}
	}
	return arr;
}
/**
...
Array.prototype.arrDelSpace = Array.prototype.arrDelSpace || function(arr){
	for(var i=0, len=arr.length; i<len;){	if(/^\s*$/.test(arr[i])){	arr.splice(i,1);	}else{	i++;	}	}	return arr;
}
...
function PicMove(id, sUl, lId, rId, option){
	...
	this.oLeft.onclick = function(){	_this.run('left');	}
	this.oRight.onclick = function(){	_this.run('right');	}
}
...
PicMove.prototype.run = function(dir){
	...
	wb.startMove(this.oUl, {'left':iLeft},function(){	_this.lock =false;	});
}
